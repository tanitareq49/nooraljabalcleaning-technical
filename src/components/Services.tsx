      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          {services.map((service, index) => (
            <div 
              key={index}
              className={`bg-card rounded-2xl shadow-soft hover:shadow-medium transition-all duration-500 overflow-hidden card-hover fade-in-up`}
            ...
            >
              ...
              <div className="relative h-48 overflow-hidden">
                ...
                {/* Icon */}
                <div className="absolute top-4 right-4 rtl:right-auto rtl:left-4 w-12 h-12 bg-white/20 backdrop-blur-sm rounded-full flex items-center justify-center">
              ...
              <div className="p-6">
                ...
                <ul className="space-y-3">
                  {service.items.map((item, itemIndex) => (
                    <li key={itemIndex} className="flex items-center space-x-3 rtl:space-x-reverse">
                      <CheckCircle className="w-5 h-5 text-primary flex-shrink-0" />
                      <span className={`text-card-foreground ${isRTL ? 'font-arabic' : 'font-english'}`}>
                        {item}
                      </span>
                    </li>
                  ))}
